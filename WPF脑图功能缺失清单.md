...
| B14 | 概要节点插入 | **`经核实 WPF 原版无此功能`**（`summary` 全仓命中均为 C# XML 文档注释 `/// <summary>`，脑图源码零命中） | `summary`；未定位到确切行号 | 将多个节点归约为概要 | 完全缺失 | B-优化建议 | 高 |
| B15 | 关联线/关系线 | **`经核实 WPF 原版无此功能`**（`relation` 在脑图代码里零命中） | `relation`；未定位到确切行号 | 跨分支连接并保存 | 完全缺失 | B-优化建议 | 高 |
