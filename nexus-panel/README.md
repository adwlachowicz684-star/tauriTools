...
| **B站 UP 主** | 检测 UP 主是否有新投稿，输出 true / false |
| **微信公众号** | 检测公众号是否有新推文，输出 true / false |

#### CLI 节点：输出参数

CLI 节点跑完后，可以把「改了哪些文件」作为**参数**传给下游。
...
| `{{n1.fileNames}}` | 全部文件名 |
| `{{n1.fileDir}}` | 所在目录 |
| `{{n1.fileRel}}` | 原始路径（未拼 workdir） |
| `{{n1.fileExt}}` | 扩展名 |
| `{{n1.fileCount}}` | 数量 |

路径会与节点的**工作目录**拼接并规范化（`./a.ts` + `/proj` → `/proj/a.ts`）。

**两种来源**：

- **自动识别**（默认）：从 CLI 输出里识别路径。能处理绝对路径、相对路径、反引号包裹、Windows 路径、中文粘连（`修改文件src/foo.ts成功`）、无扩展名的 `Dockerfile`/`Makefile`、`.gitignore` 这类隐藏文件；会自动挡掉 URL、版本号、时间、普通单词
- **手动指定**：识别不准时改为一行一个路径

面板里有**实时预览**，能看到当前配置下会识别出哪些文件，不必先跑一遍。

**自定义参数**：给输出里的某个值起个名字，三种来源 —— 文件路径 / 正则捕获 / 固定值，可增删改与开关。正则非法时取空串，不会中断流程。

> 注意：识别是**尽力而为**的文本匹配，没有标准答案。关键链路建议用「手动指定」或让 agent 在输出里明确写出路径。

#### OCR / 翻译节点（调用大模型 API）

两个节点直接调大模型，不经过 CLI，因此**不消耗 Trae / WorkBuddy 积分**，用的是你自己填的 API 额度。

**OCR（图片识别）** — 把图片里的文字识别出来

- 图片来源：网络地址 / 本地文件
- 需要**支持视觉的模型**（GPT-4o、GLM-4V、qwen-vl 等）；选了 DeepSeek、Moonshot 这类不支持图片的会直接提示
- 细节档位：自动 / 低（省 token）/ 高（识别更准）

**翻译** — 把文本译成目标语言

- 目标语言：8 个预设 + 自定义（可写「简练的文言文」这类说法）
- 源语言留空让模型自动判断
- **术语表**：每行一条 `GPU=图形处理器`，保证专有名词译法一致

模型被要求**只输出译文**，不加解释、不加代码块标记，结果可直接喂给下游。

**服务商预设**（都兼容 OpenAI 的 `/chat/completions` 格式）：

| 服务商 | 默认模型 | 支持图片 |
|---|---|---|
| OpenAI | gpt-4o-mini | ✓ |
| DeepSeek | deepseek-chat | ✗ |
| 智谱 GLM | glm-4-flash | ✓（视觉用 glm-4v 系列） |
| Moonshot | moonshot-v1-8k | ✗ |
| 通义千问 | qwen-plus | ✓（视觉用 qwen-vl-max） |
| 硅基流动 | Qwen2.5-7B-Instruct | ✓（模型名含 VL） |
| 自定义 | — | — |

**两点注意**：

1. **API Key 随画布一起保存**。导出画布分享给别人前记得清掉
2. 请求经 Tauri 的 http 插件发出以绕过 CORS。**若外壳未启用该插件**，会退回浏览器 fetch，多数大模型 API 会因跨域被拒绝 —— 这时需要在 Rust 侧启用 `tauri-plugin-http`

#### 条件分支节点

判定上游文本，走不同分支。不调用 CLI、不消耗积分。

面板里是**可视化构建器**，每块都能看懂：

| 区块 | 作用 |
|---|---|
| **试跑** | 粘一段示例文本，立刻看到会命中哪条分支；规则卡片上标「命中 / 未中 / 跳过」 |
| **表达式预览** | 把规则渲染成 `来源 ⊇ 包含 「值」` 这样的彩色 chip，一眼看懂 |
| **算子选择** | 按「文本比对 / 空值判断 / 流程控制」分组的图标按钮，替代纯文字下拉 |
| **语义提示** | 每个算子下方写明精确行为 |
| **配置提示** | 实时检查，见下 |
| **条件列表** | 每条规则下的多条条件，各自可选算子/值/来源，可单独开关与删除 |

**9 种算子**（图标即含义）：

| 分类 | 算子 |
|---|---|
| 文本比对 | `⊇` 包含 · `⊉` 不包含 · `=` 等于 · `≠` 不等于 · `↦` 开头是 · `.*` 正则匹配 |
| 空值判断 | `●` 非空 · `○` 为空 |
| 流程控制 | `✓` 总是 |

**多条件组合**：一条规则可含多条条件，用「且（AND）/ 或（OR）」组合，条件可单独**添加 / 删除 / 修改 / 开关**。

- 开关在调试时最有用：临时停一条比删了重建省事
- AND 下所有启用的条件都成立才命中；OR 下任一成立即命中
- 非法正则那条会被排除在组合之外（不中断流程），停用它则不再报错
- 停用的条件不参与组合：一条「包含 A」+ 一条停用的「包含 B」，AND 下只按 A 判定

**三个反直觉行为，面板会主动提示**：

1. **比较值留空时「包含 / 不包含 / 正则匹配」恒为真** —— 不是"不匹配"。想判断有没有内容请用「非空」
2. **`等于` 会先去掉首尾空格**，`开头是` 只去首部空格
3. **正则写错只跳过该规则**，不会中断流程

另外「总是」若不在最后一条，它后面的规则永远不会执行；未启用兜底时，所有规则都不命中会让下游**全部静默跳过**——这两种情况面板都会警告。

#### 触发器节点

节点库里只有**一个**「触发器」项，但一个节点可以**同时挂多种**触发方式：

手动 / 周期 / 定时（cron）/ 监听目录 / 调用（webhook）

在右侧面板的「触发方式」里勾选即可，任一满足就触发。
比如同时勾「周期 + 监听」：定时兜底，改文件立刻跑。

实现上的一个关键点：五种方式共用同一个 Config 对象、各取所需字段，
所以勾选/取消**不会清空已填的配置**（填好的监听目录切走再勾回来还在）。

调度时，一个节点会按勾选的方式展开成多条 trigger（id 形如 `nodeId:kind`），
调度器内部按 id 记录"上次触发时刻""cron 下次时刻""防抖定时器"，
因此多种方式各自独立计时、互不覆盖；
而全局防重入仍然只有一把锁——多选也不会并发跑。

默认只勾「手动」：周期、定时、监听、调用都会自动跑，放上画布就生效太危险。

#### 更新检测节点（B站 / 公众号）

两个节点：B站 UP 主、微信公众号。都输出 `true` / `false`，接条件节点即可分流。

条件节点来源选这个节点，判定「等于 `true`」。
...
**工作原理**：抓取 → 解析 → 与上次记录的基线比对。基线存在节点数据里，
随画布持久化，重启后仍记得上次看到的是哪一条。

首次运行默认只记基线、**不算更新**（否则刚配好就会触发一次下游）。
可在面板里改，或点「重置基线」重来。
...
| 方式 | 说明 |
|---|---|
| **RSS（推荐）** | 填 RSSHub 之类的地址，如 `https://rsshub.app/bilibili/user/video/{UID}` |
| 官方接口 | 风控严格，通常需要完整 Cookie + Referer |

⚠️ 老接口 `/x/space/arc/search` 与 `/x/space/arc/list` 已下线，
现在必须用 wbi 签名版，不带签名会被直接拒绝。所以**官方接口模式并不省心**——
报 `-352` / `-403` 时请改用 RSS。

只填 SESSDATA 往往不够，B站 还会看 `buvid3` / `_uuid`，
建议把浏览器里整条 Cookie 粘进来。

##### 微信公众号

公众号**没有官方开放接口**，必须填第三方桥接生成的订阅地址：

- wechat2rss
- RSSHub（`/wechat/...` 路由）
- Feeddd 等

免费源常有调用次数限制，失效时换一个即可。

##### 共用设置

- 自定义 User-Agent（部分源拒绝非浏览器 UA）
- 超时秒数
- 输出格式：`bool`（只输出 true/false）/ `detail`（附带标题链接）
- **测试连接**按钮：抓取并判定，但**不会更新基线**

#### 循环节点

> **已修（B1–B4）**：审查发现的四个循环相关问题均已修复，详见下方「循环节点的四个坑」。
...
bash scripts/run-tests.sh     # 217 项
```
