...
const fields: FieldDef[] = [
  ...
  },
  {
    ...
    key: 'relaySession',
    label: '',
    placeholder: '接着同一个会话跑（-c）',
    /*
     * 这两句必须写清楚，否则开关就是个"拨了看不出区别"的摆设：
     *
     * 1. 只有 codebuddy 支持。traecli 官方没有恢复参数，
     *    开了也不带 —— 界面上却看不出"这一条没生效"。
     * 2. "上一次"是 CLI 全局记的。用户在自己终端里刚跑过同一个 CLI，
     *    接力的可能是那一次，表现为节点读到了一段没见过的历史。
     */
    hint: '仅 codebuddy 支持（traecli 没有恢复参数，开了也不带）。接的是 CLI 自己记的「上一次」，若你在别处刚跑过同一个 CLI，接力的可能是那一次；成员之间也会变成强串行',
  },
];
