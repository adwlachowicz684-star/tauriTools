import { registerVariableGroup, type VariableGroupDef } from '../engine/variables';

/**
 * 内置变量组。
 *
 * 新增一组变量在这里注册一条即可，不用改任何节点 ——
 * 节点只是声明「我支持哪些组」（meta.varGroups）。
 */

const asText = (v: unknown): string => String(v ?? '').trim();

/** GitHub 仓库地址：owner / repo / branch */
export const GITHUB_REPO_GROUP: VariableGroupDef = {
  group: 'github-repo',
  label: '地址变量',
  ...
  /*
   * 至少要有 repo 才算一个能用的地址变量。
   * 放宽到"三项都必填"会误伤：更新检测节点允许留空 branch（用默认分支），
   * 那是正当用法，不该被卡住。
   */
  validate: (v) => (asText(v.repo) ? null : '没填仓库名，引用上去也是空的'),
};

/** HTTP 请求地址：url / method */
export const HTTP_ENDPOINT_GROUP: VariableGroupDef = {
  group: 'http-endpoint',
  label: '接口变量',
  keys: ['url', 'method'],
  name: '接口',
...
export const LLM_CONFIG_GROUP: VariableGroupDef = {
  group: 'llm-config',
  label: '模型变量',
  keys: ['llm'],
  name: '模型',
...
/** 命令工作目录（CLI 类节点） */
export const WORKDIR_GROUP: VariableGroupDef = {
  group: 'workdir',
  label: '目录变量',
  keys: ['workdir'],
  name: '工作目录',
...
/** 幂等：多处 import 也只注册一次 */
export function registerBuiltinVariableGroups(): void {
  if (done) return;
  done = true;
  registerVariableGroup(GITHUB_REPO_GROUP);
  registerVariableGroup(HTTP_ENDPOINT_GROUP);
  registerVariableGroup(LLM_CONFIG_GROUP);
  registerVariableGroup(WORKDIR_GROUP);
}
