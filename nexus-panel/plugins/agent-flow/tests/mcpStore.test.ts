...
import {
  ...
  refreshAll, describeRefresh, collectServers, MCP_BP_KEY,
  type McpToolFetcher,
...
test('没有协议实现 → 退化成用快照，并说明原因', async () => {
  ...
  const r = await refreshAll({ servers: [{ name: 'fs' }], previous: prev });
...
test('没变化 / 有变化 / 失败 三种文案要说不同的话', async () => {
  ...
  const failed = await refreshAll({ servers: [{ name: 'fs' }], previous: prev });
