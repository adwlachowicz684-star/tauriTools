...
test('普通点击要有反馈（打开说明浮层），不是静默无反应', () => {
  /*
   * 以前断言的是 setOpenKey（在列表里展开说明块）。
   * 说明改成浮层后，反馈变成打开浮层 —— 断言跟着换成 setTip，
   * 但**守的还是同一件事**：单击必须有看得见的反应。
   *
   * 静默会让人以为界面坏了，进而反复点击，反而更容易误触发添加。
   */
  assert.ok(
    /setTip/.test(CODE),
    '普通点击应打开说明浮层；静默无反应会让人以为界面坏了从而反复点击',
  );
});
