...
| 右 · 属性侧栏 | 主题 / 标签 / 样式 / 文件 四页 | 宽度取 C# 的 `Width="276"`；**页签位置不同**，见下 |
| 底 · 页签条 | 画布页签 + 最右 `＋` | 一致（C# 也是 `ScrollViewer` + 右侧 `＋`） |
| 底 · 状态条 | 操作反馈 | 一致（C# 在 `Grid.Row=3`） |

**页签位置是唯一有意偏离 C# 的地方**：C# 把四个 `ToggleButton` 放在顶栏最右
（`Grid.Column=14`）。侧栏改为常驻后，页签若留在顶栏最右，视线要横穿整个画布
才能把「点了哪个钮」和「右侧变成什么」对上；放在内容正上方一目了然。
