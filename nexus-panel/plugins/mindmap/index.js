...
bootIframePlugin(async (ctx) => {
  ...
  function buildToolbar() {
    ...
    // 字体 / 字号
    const sizeSel = h('select.mm-select', {
      onchange: (e) => { bridge?.exec('fontsize', Number(e.target.value)); commit(); },
    }, ...[12, 14, 16, 18, 20, 24, 28, 32, 40].map((n) => h('option', { value: n }, String(n))));

    const fontSel = h('select.mm-select', {
      onchange: (e) => { bridge?.exec('fontfamily', e.target.value); commit(); },
    }, ...['微软雅黑', '宋体', '黑体', '楷体', 'Arial', 'Consolas'].map((f) =>
      h('option', { value: f }, f)));

    // 前景色
    const colorInput = h('input', {
      type: 'color', value: '#333333',
      style: { position: 'absolute', inset: '0', opacity: '0', width: '100%', height: '100%', cursor: 'pointer' },
      oninput: (e) => { bridge?.exec('forecolor', e.target.value); commit(); },
    });
    const colorBtn = h('button.mm-swatch', {
      style: { background: '#333333', position: 'relative' },
      title: '文字颜色',
    ...
    toolbar.appendChild(group(
      h('button.mm-btn', {
        ...
        onclick: () => { bridge?.exec('bold'); commit(); },
      }, 'B'),
      h('button.mm-btn', {
        style: { fontStyle: 'italic' },
        onclick: () => { bridge?.exec('italic'); commit(); },
      }, 'I'),
      h('button.mm-btn', {
        style: { textDecoration: 'line-through' },
        onclick: () => { bridge?.exec('strikethrough'); commit(); },
      }, 'S'),
      fontSel,
      sizeSel,
      colorBtn,
      // 水平对齐：textalign；垂直对齐：valign（C# 版 BtnValignT/C/B 三个按钮）。
      // 注：C# 侧另有 Vo* 九个滑块（3 层级 × 3 对齐的像素微调），那套是临时调试脚手架，
      // 且「固化」时会用正则改写 index.html 源码 —— Web 沙箱无权改本地文件，故不移植。
      B('左', () => { bridge?.exec('textalign', 'left'); commit(); }, { icon: true, title: '水平左对齐' }),
      B('中', () => { bridge?.exec('textalign', 'center'); commit(); }, { icon: true, title: '水平居中' }),
      B('右', () => { bridge?.exec('textalign', 'right'); commit(); }, { icon: true, title: '水平右对齐' }),
      B('上', () => { bridge?.exec('valign', 'top'); commit(); }, { icon: true, title: '文字靠上对齐' }),
      B('垂中', () => { bridge?.exec('valign', 'middle'); commit(); }, { icon: true, title: '文字垂直居中' }),
      B('下', () => { bridge?.exec('valign', 'bottom'); commit(); }, { icon: true, title: '文字靠下对齐' }),
    ));
