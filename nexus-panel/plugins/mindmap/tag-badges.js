...
/**
 * 徽章按钮的值序列：**单排 10 格**（1–9 + 清除）。
 *
 * 原先是 `[[1..5], [6,7,8,9,0]]` 两排（对齐 WPF `BuildSpriteNumRow` 的两行）。
 * 改成一排是因为：这是**同一个维度**的 10 个档位（优先级 1–9 + 清除），
 * 拆成两排会让人误以为 1–5 和 6–9 是两类不同的东西，选档位时视线要
 * 跳两行；而且两排各 5 个、右边空一大片，看着像没排满。
 *
 * 宽度是够的：侧栏 276px − 左右 padding 24px = 252px 可用；
 * 10 × 20px 徽章 = 200px，余 52px 分给 9 个间隙（约 5.8px/个）。
 * 渲染上用 `space-between` 铺满，**不换行** —— 万一将来徽章变宽
 * 也只是间隙被压掉，不会掉到第二排。
 *
