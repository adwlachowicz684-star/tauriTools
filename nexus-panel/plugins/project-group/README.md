...
| **发送到 AI** | 内置四项连锁动作（自由任务 / 一键审查 / 快速归并 / 快速部署）+ 自定义；每个动作分别有项目与项目组两份模板，可指定专属客户端、可挂右键菜单。发给 opencode / Cursor / VSCode / Trae / WorkBuddy 等 10 个客户端（部分客户端降级为复制+粘贴；WorkBuddy 的 deep link 只做界面导航、不接收任务，所以走的是唤起 + 复制这一路） |
| **模板占位符** | 模板编辑框下方有插入按钮，点一下插到光标处（有选中时替换选中部分）。支持 `{path}` `{name}`（本插件写法）与原版 `{项目路径}` `{项目名称}`，另有 `{工具根目录}` `{工具路径}` `{工具文件名}`；未识别的 `{…}` 原样保留 |
...
| **界面快捷键** | F1 使用说明 / F5 刷新 / F7 一键备份 / F8 清除无效项 / Ctrl/⌘+M 启停 MCP / Ctrl/⌘+D 编辑内容区选中的文件 / Shift+~ 收起左操作栏 |
| **快捷键可自定义** | 设置 → 快捷键：点键位后按下新组合键，Esc 放弃、Del 取消绑定、↺ 恢复默认；重复键位会当场标红。分三组展示（常规 / 项目操作 / 页签切换） |
| **左操作栏可收起** | Shift+~ 或栏底按钮。收起后仍留一条窄栏与展开按钮——不做成彻底隐藏，否则收起就没有可见的回头路 |
| **防写入档下的自身写点** | 写 SKILL.md / desktop.ini 都经 `with_unlock` 临时摘锁窗口：按**祖先**摘（ACL 靠继承传播，要写的常是子孙，精确相等查不到）；窗口要小，绝不盖住 spawn 出去的外部进程；恢复失败绝不静默（摘了没恢复 = 目录永久失去保护） |
| **图标路径的空格与逗号** | desktop.ini 的 `IconResource` 路径含空格或逗号时加双引号包裹（含空格的路径是常态，此前 Shell 会按空格截断、图标**静默失效**）；不带空格/逗号的路径与修复前字节完全一致 |
| **移动期间抑制监控器** | 改名 / 搬家前登记抑制，监控线程在那一个轮询周期内**照常更新指纹但不报事件**。不抑制的话用户改个名就被自己的告警吓一次；不更新指纹的话抑制结束后会把差异一次性全报出来，等于只是把误报推迟 |
| **配置体检** | config.json 加了 schema 版本；启动时检查**不被识别的键**并列出键名（此前这类旧键会被 serde 静默忽略 —— 字段改名后用户的数据就悄悄没了，清单 #454 记过一次 clusters）。合法字段集由序列化自身推导，不手写清单 |
| **迁移命令的预演与备份** | `--migrate-hierarchy` / `--migrate-groups-hierarchy` 支持 `--dry-run`（只算计划不动磁盘）；执行前自动把 config 与 link-record 各留一份带时间戳的 .bak 副本，**不覆盖已有备份**（连跑两次时第二份才是有用的现场）。迁移会物理搬目录并覆写 config，此前没有任何测试 |
| **内置 Markdown 编辑器** | Ctrl/⌘+D 打开共享服务 `md-editor` 编辑当前条目：读文本 → 编辑 → 写回，全程不出本工具（此前走外部编辑器，相关注释已陈旧）。**取消的两种形态都认**（服务约定 resolve(null)，实现却是 reject）—— 只认一种的话，用户点个取消就会 unhandled rejection。内置编辑器没返回结果时退回外部编辑器，不报红 |
| **链接的磁盘兜底** | 账本里没有记录时，扫描项目目录下**真实存在的链接**反查项目组（链接可能是手工 mklink、别的脚本、旧版本迁移遗漏时建的）。只认有效链接，优先取已登记的组路径；有账本记录时不覆盖。**跳转也一并兜底** —— 否则会出现"组名看得见、点它却报错"，比干脆不显示更让人困惑 |
| **备份目录可分别指定** | 设置页「备份」新增三个目录行：备份根目录（两类共用）、项目备份目录、项目组备份目录。此前字段与后端都有，却只有「一键备份」弹窗里能改，设置页只有两个「打开」按钮 —— 想分别指定只能手改 config.json（与 #49 同一类缺口） |
| **按钮上显示快捷键** | 工具栏的备份 / 使用说明 / 刷新 / 清除无效项按钮上直接显示当前键位，设置页「界面」可关闭（默认开）。**键位从快捷键设置动态取、不手抄**；用户取消某个绑定后按钮上不再显示任何提示（而不是退回默认值——那样用户照着按却没反应） |
| **客户端刷新检测** | 设置页「Agent 连锁」的「刷新检测」按钮：装完新客户端回来点一下重新检测，结果显示检测到哪几个。**只剩 opencode 时会特别说明**——后端一个都没检出时会拿它兜底，不点破的话用户会以为自己装好了 |
| **开发者模式 + 恢复默认** | 设置页「高级」里的开发者模式默认关，开启后允许删除内置连锁动作（默认拦住是因为删了不会自己回来 —— `ensure_actions` 只在清单为空时才重建）。内置动作的图标与名称都带「恢复默认」；自定义动作随时可删、图标默认 🧩 |
| **发送前确认** | 点发送先弹确认框，显示**占位符已替换后的指令全文**且可编辑，带「本次运行期间不再提示」勾选项。面板入口与侧边栏/快捷键入口**共用同一个确认框**（只做一个的话，另一条路就完全绕过了确认）；预览失败会降级为直接发送而不卡住 |
| **页签管理** | 工具栏「页签管理」：项目 / 项目组两栏页签集中一处改名、↑↓ 排序、删除；每行显示条目数与受保护数（删页签会连带移除这些登记，不给数不知道损失了什么）。页签条上的 ⋮ 菜单与拖拽排序仍在 |
| **跳转后滚动定位** | 从项目跳到项目组时，若目标所在分类是折叠的会先展开，再把卡片滚进视野 —— 项目组栏是所有分类纵向堆叠，只选中不滚动的话高亮常在屏幕外，用户会以为"跳转没反应" |
| **目录型 skill 双击开 SKILL.md** | 双击目录型 skill 直接打开其下的 SKILL.md，而不是停在目录那一层 |
| **链接名批量操作** | 「全选 / 全不选」一键切换启用；「恢复预设」清空改名与厂商标注、名字回到预设原名，而**置顶与备注保持不变**（它们的键是显示名，名字变回原名时会跟着迁键，不会静默失效）。未置顶的图钉平时隐藏，悬停或键盘聚焦该行时显形 |
| **布局可调并记忆** | 拖栏间分隔条调三栏宽度、拖日志区上方分隔条调高度、拖浮层底边调面板高度；松手即记忆。方向键可微调（Shift 加速）。拖乱了在「设置 → 布局」点「恢复默认布局」。窗口尺寸记忆属宿主能力，不在本插件 |
...
- **快捷键有两条让路规则**：焦点在 input / textarea / select 里时不触发（否则改名框里按 Delete
  ...
  **例外：开关类键位（F1 使用说明 / Ctrl+⌘+M 启停 MCP / Shift+~ 收起左栏）不受第二条限制**——
  若也跟着让路，F1 打开说明后 `help` 一置位就再也关不掉，等于把自己废了。
