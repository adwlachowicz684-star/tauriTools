...
export function Dialogs(props: DialogsProps) {
  ...
  /*
   * `a ?? b ? c : d` 的实际解析是 `(a ?? b) ? c : d` —— ?? 与三元混用时
   * **不会**按直觉先算三元。少了括号的后果：
   *   跟随模式下只要 selCard?.path 非空，就会无条件取 dialog.card.path，
   *   而 dialog 此时可能是 'none' / 'pickDir' 等没有 card 字段的成员
   *   → 读 undefined.path，运行时 TypeError。
   * 下一行 iconTargetName 写了括号，两行写法不一致正是漏写这里的原因。
   */
