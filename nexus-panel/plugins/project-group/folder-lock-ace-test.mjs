...
console.log('\n=== #317 摘锁窗口的并发边界说明 ===');
{
  ...
  t('注释：说明了对本工具为什么安全（GUI 单线程 + MCP 串行）',
    /GUI 单线程 \+ MCP 单请求串行/.test(blk));
  t('注释：明确警告不要放进多线程热点路径',
...
console.log('\n=== #414 项目搬家只改账本的 project 字段 ===');
{
   ...
   */
  /* 这条原先锚在注释「只有项目组搬家需要重建」上：注释一改写就假失败，
     而它真正要守的"项目不重建"其实已被下一条（两处都带守卫）覆盖。
     改成纯代码断言：两处重建循环都必须被 kind_is_group 包住。 */
  t('两处重建都带守卫（改名那处也钉住）',
