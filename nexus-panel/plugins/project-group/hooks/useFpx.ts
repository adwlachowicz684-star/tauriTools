...
export function useFpx() {
  ...
  const syncLinks = useCallback(async (project: string, group: string, names: string[]) => {
    ...
    if (snap) {
      ...
      /*
       * 后端带回的**附加说明**（不是错误）：某些名字被普通目录/文件占用，
       * 为免误删内容已跳过。
       *
       * 必须显示出来：操作本身成功了，但用户勾的名字确实没全建 ——
       * 不说的话他以为勾了就建好了，而那个名字一直没动。
       * 记日志而不只 toast：toast 一闪就消失，这类"要手动处理"的事
       * 需要留一条可回看的记录。
       */
      for (const msg of snap.linkNotices ?? []) pushLog(`同步链接：${msg}`, true);
      ctx.toast('链接已同步', 'ok');
    }
  }, [api, applySnapshot, ctx, pushLog, run]);
