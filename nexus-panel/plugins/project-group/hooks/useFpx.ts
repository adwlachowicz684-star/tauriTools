...
export function useFpx() {
  ...
  const createLink = useCallback(async (project: string, group: string, names?: string[]) => {
    ...
    if (snap) {
      applySnapshot(snap);
      /*
       * 按**归一化键**找这一行，不能用 `l.project === project`。
       *
       * 后端按归一化键匹配（Windows 下大小写不敏感 + 去尾分隔符），
       * 而这里是原文精确比。配置被手改过、或路径来源不同时
       * （比如拖进来的路径带尾反斜杠），find 返回 undefined →
       * 日志写成「已分配：X → Y（0 个链接）」。
       *
       * 用户看到"分配完成"配着"0 个链接"，只能以为这次分配没生效，
       * 于是再点一次 —— 而链接其实已经建好了（第二次是重复操作）。
       * 这类"报告的数字与事实不符"不报错，只会让用户做错后续判断。
       */
      const k = normalizeKey(project, ci);
      const row: LinkRow | undefined = snap.links.find((l) => normalizeKey(l.project, ci) === k);
      pushLog(`已分配：${project} → ${group}（${row?.names.length ?? 0} 个链接）`);
      ctx.toast('分配完成', 'ok');
    }
  }, [api, applySnapshot, ci, ctx, pushLog, run]);
